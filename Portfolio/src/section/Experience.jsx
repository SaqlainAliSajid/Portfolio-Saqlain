...
const scale = useTransform(scrollYProgress,[start,end,1],[0,1,1],{clamp:true});
  ...
  if(layout==="desktop"){
    return(
      <div className="relative flex flex-1 justify-center items-center min-w-0">
        <motion.div className="z-10 w-6 h-6 xl:w-7 xl:h-7 rounded-full bg-white shadow-[0_0_0_8px_rgba(255,255,255,0.1)]"
        style={{scale,opacity}}
        ></motion.div>
        ...
        backdrop-blur border border-gray-700/70 rounded-xl p-5 xl:p-6 w-64 xl:w-70 shadow-lg `}
        style={{opacity ,y ,maxWidth:"90vw"}}
        transition={{duration:0.4 , delay:idx*0.15}}
        >
          <h3 className="text-base xl:text-lg font-semibold leading-tight wrap-break-word">{exp.role}</h3>
          <p className="text-xs xl:text-sm text-gray-400 mb-2">{exp.company}</p>
          <p className="text-xs xl:text-sm text-gray-400 mb-2">{exp.duration}</p>
          <p className="text-xs xl:text-sm text-gray-300 wrap-break-word">{exp.description}</p>
        </motion.article>
      </div>
    )
  }
  return(
    <div className="relative flex items-start">
      <motion.div className="absolute -left-[14px] top-3 z-10 w-6 h-6 rounded-full bg-white shadow-[0_0_0_8px_rgba(255,255,255,0.1)]"
      style={{scale,opacity}}></motion.div>
      <motion.article className="bg-gray-900/80 backdrop-blur border border-gray-700/70 rounded-xl p-4 sm:p-5 w-[88vw] max-w-xs sm:max-w-sm ml-6 shadow-lg"
      style={{opacity,x}}
      transition={{duration:0.4 , delay : idx*0.15}}
      >
        <h3 className="text-sm sm:text-base font-semibold wrap-break-word leading-tight">{exp.role}</h3>
          <p className="text-xs sm:text-sm text-gray-400 mb-2 wrap-break-word">{exp.company}</p>
          <p className="text-xs sm:text-sm text-gray-400 mb-2 wrap-break-word">{exp.duration}</p>
          <p className="text-xs sm:text-sm text-gray-300 wrap-break-word">{exp.description}</p>
      </motion.article>
    </div>
  )
    ...
    <section id="experience" className="relative bg-black text-white">
      ...
      <div className="relative" ref={sceneRef} style={{height : `${SCENE_HEIGHT_VH}vh`, minHeight:"120vh"}}>
        <div className="sticky top-0 h-screen flex flex-col">
          <h3 className="text-3xl sm:text-4xl lg:text-5xl font-semibold mt-5 text-center">Experience</h3>
          <div className="flex flex-1 items-center justify-center px-6 pb-10">
            {!isMobile && (
              <div className="relative w-full max-w-7xl">
                ...
                <div className="relative justify-between flex gap-3 xl:gap-4 mt-0">
            ...
            {isMobile && (
              <div className="relative w-full max-w-md">
                ...
                <div className="relative flex flex-col pb-28 gap-8 sm:gap-10 ml-10 mt-6">
                  {visibleExperiences.map((experiences,idx)=>(
                  <ExperienceItem
                  key={idx}
                  exp ={experiences}
